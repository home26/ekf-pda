function [ P,x,measurement,trackState,qualityOfTrack,numberOfTrack ] = TrackManagement( P0,x0,measurement_0,trackState_0,qualityOfTrack_0,numberOfTrack, meaCandidate,numberOfmea)
...
beta = zeros(1,20);%define the paobablity matrix for every measurement_0 
sumOfLikelihoodRatio = 0; %define the sume of likelihoodratio
innovation = zeros(2,20);%define the innovation
sumOfInnovation = zeros(2,1);%define the sume of innovation
sumOfTempOfP = zeros(2,2);
...
for i = 1:numberOfTrack
    if trackState_0(:,:,i) ~= 2
        ...
        H = zeros(2,4);
        H(1,:) = [(x_(1,1,i)-600)/(sqrt((x_(1,1,i)-600)^2+(x_(3,1,i)-600)^2)),0,(x_(3,1,i)-600)/(sqrt((x_(1,1,i)-600)^2+(x_(3,1,i)-600)^2)),0];
        H(2,:) = [(600-x_(3,1,i))/((x_(1,1,i)-600)^2+(x_(3,1,i)-600)^2),0,(x_(1,1,i)-600)/((x_(1,1,i)-600)^2+(x_(3,1,i)-600)^2),0];
    
        %define S matrix
        S = H*P_(:,:,i)*transpose(H) + R;
    
        %z_hat matrix
        z_hat(1,1) = sqrt((x_(1,1,i)-600)^2+(x_(3,1,i)-600)^2);
        z_hat(2,1) = atan2((x_(3,1,i)-600),(x_(1,1,i)-600));
        ...
        if numberOfAssocaition > 0 %if measurements fell into the gate
            ...
            for q = 1:numberOfAssocaition
                beta(1,q) = likelihoodRatio(1,q)/(1-PD*PG+sumOfLikelihoodRatio);
                innovation(:,q) = meaAssociated(:,q) - z_hat;
                sumOfInnovation = sumOfInnovation + beta(1,q)*innovation(:,q);
            end

            x(:,:,i) = x_(:,:,i) + W*sumOfInnovation;%update the state
        
            beta0 = (1-PD*PG)/(1-PD*PG+sumOfLikelihoodRatio);
            %%%%%%%update the P matrix%%%%%%%%
            Pc = P_(:,:,i) - W*S*W';
            for m = 1:numberOfAssocaition
            sumOfTempOfP = sumOfTempOfP+beta(1,q)*innovation(:,q)*innovation(:,q)'-sumOfInnovation*sumOfInnovation';
            end
            Pdelta = W*sumOfTempOfP*W';
            P(:,:,i) = beta0*P_(:,:,i) + (1-beta0)*Pc + Pdelta;
            %%%%%%%update the P matrix%%%%%%%%
            ...
            if trackState_0(:,:,i) == 0
                if qualityOfTrack(:,:,i) >= 0.5
                    trackState(:,:,i) = 1;
                elseif qualityOfTrack(:,:,i) < 0.0000
                    trackState(:,:,i) = 2;
                end
            end
        
            if trackState_0(:,:,i) == 1
                if qualityOfTrack(:,:,i) <= 0.0000
                    trackState(:,:,i) = 2;
                end
            end
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    innovation = zeros(2,20);
    sumOfInnovation = zeros(2,1);
    sumOfTempOfP = zeros(2,2);
    numberOfAssocaition = 0;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
