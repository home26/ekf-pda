function [ P,x,measurement,trackState,qualityOfTrack,numberOfTrack ] = TrackManagementXY( P0,x0,measurement_0,trackState_0,qualityOfTrack_0,numberOfTrack, meaCandidate,numberOfmea)
...
for i = 1:numberOfTrack
    if trackState_0(:,:,i) ~= 2
        ...
        if numberOfAssocaition > 0 %if measurements fell into the gate
            ...
            sumOfTempOfP = sumOfTempOfP+beta(1,m)*innovation(:,m)*innovation(:,m)';
            end
            sumOfTempOfP = sumOfTempOfP - sumOfInnovation*sumOfInnovation';
            Pdelta = W*sumOfTempOfP*W';
            P_up = beta0*P_(:,:,i) + (1-beta0)*Pc + Pdelta;
            P_(:,:,i) = P_up;
            %%%%%%%update the P matrix%%%%%%%%
...
for n = 1:numberOfmea
    if meaState(1,n) == 0
        [P_tmep,x_temp,measurement_temp,trackState_temp,qualityOfTrack_temp,numberOfTrack_temp] = InitializationXY(meaCandidate(:,n) , 1);
